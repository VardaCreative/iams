...
const SidebarItem = ({ icon: Icon, label, href, isCollapsed }: SidebarItemProps) => {
  ...
  return (
    <Link
      to={href}
      className={cn(
        "flex items-center gap-3 rounded-lg px-3 py-2 transition-all duration-250 group relative overflow-hidden",
        isActive 
    ...
    >
      ...
      {isCollapsed && isActive && (
        <div className="absolute inset-y-0 -right-3 w-1 bg-primary-foreground rounded-l-full" />
      )}
      <div className={cn(
        "absolute inset-0 bg-primary opacity-0 transition-opacity duration-350",
        isActive ? "opacity-[0.08]" : "group-hover:opacity-[0.03]"
      )} />
    </Link>
  );
};
...
const Sidebar = () => {
  ...
  return (
    <div 
      className={cn(
        "h-screen flex flex-col border-r border-border bg-card transition-all duration-350 shadow-sm relative",
        isCollapsed ? "w-16" : "w-64"
      )}
    >
      <div className="p-4 flex items-center justify-between border-b">
        {!isCollapsed && (
          <h2 className="font-bold text-xl text-foreground tracking-tight">Spice ERP</h2>
        )}
        {isCollapsed && (
          <span className="font-bold text-xl text-foreground mx-auto">S</span>
        )}
        <button
          onClick={toggleSidebar}
          className="p-1 rounded-md text-muted-foreground hover:bg-secondary hover:text-foreground transition-colors duration-250"
        >
      ...
      <div className={cn(
        ...
        <div className="flex items-center gap-3">
          <div className={cn(
            "h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center text-primary",
            isCollapsed ? "mx-auto" : ""
          )}>
